...
function getDmg(spellname,target,owner,stagedmg,spelllvl)
	...
	elseif spellname == "Q" or spellname == "W" or spellname == "E" or spellname == "R" or spellname == "P" or spellname == "QM" or spellname == "WM" or spellname == "EM" then
		...
		elseif name == "Akali" then
			if spellname == "P" then DmgM = (6+ap/6)*ad/100
			...
			elseif spellname == "E" then DmgP = 25*Elvl+5+.4*ap+.6*ad
		...
		elseif name == "Annie" then
			...
			elseif spellname == "R" then DmgM = math.max((125*Rlvl+50+.8*ap)*stagedmg1,(10*Rlevel+10+.2*ap)*stagedmg2,(125*Rlvl+50+.8*ap)*stagedmg3) DmgP = (25*Rlvl+55)*stagedmg2 --stage1:Summon Tibbers . stage2:Aura AoE xsec + 1 Tibbers Attack. stage3:Summon Tibbers
			end
		elseif name == "Ashe" then  -- script doesn't calculate autos and therefore doesn't take Ashe's crit mechanic on slowed targets into effect
			if spellname == "Q" then DmgP = (.05*Qlvl+.1)*ad TypeDmg = 2 --xhit (bonus)
			elseif spellname == "W" then DmgP = 10*Wlvl+30+ad
			elseif spellname == "R" then DmgM = 175*Rlvl+75+ap
			end
		elseif name == "Azir" then
			if spellname == "Q" then DmgM = 20*Qlvl+45+.5*ap --beyond the first will deal only 25% damage
		...
		elseif name == "Bard" then
			if spellname == "P" then DmgM = 30+.3*ap  --I don't know how to check Meep count to calculate damage
			elseif spellname == "Q" then DmgM = 45*Qlevel+35+.65*ap
			end
		...
		elseif name == "Evelynn" then
			if spellname == "Q" then DmgM = 15*Qlvl+25+(.05*Qlvl+.3)*ap+(.05*Qlvl+.45)*bad
		...
		elseif name == "Fizz" then
			if spellname == "Q" then DmgM = 15*Qlvl-5+.3*ap TypeDmg = 2 -- (bonus)
			elseif spellname == "W" then DmgM = math.max(((20*Wlvl+10+.7*ap)+(Wlvl+3)*(tmhp-thp)/100)*(stagedmg1+stagedmg3),((10*Wlvl+10+.35*ap)+(Wlvl+3)*(tmhp-thp)/100)*stagedmg2) TypeDmg = 2 --stage1:when its active. stage2:Passive. stage3:when its active
		...
		elseif name == "Graves" then
			if spellname == "Q" then DmgP = math.max(30*Qlvl+30+.75*bad,(30*Qlvl+30+.75*bad)*2*stagedmg3) --xbullet , 50% damage xeach bullet beyond the first. stage3: Max damage
		...
		elseif name == "Irelia" then
			...
			elseif spellname == "E" then DmgM = 40*Elvl+40+.5*ap
		...
		elseif name == "Jayce" then
			...
			elseif spellname == "W" then DmgT = 10*Wlvl+60 --% damage
		...
		elseif name == "Jinx" then
			...
			elseif spellname == "R" then DmgP = math.max(((50*Rlvl+75+.5*bad)*2+(0.05*Rlvl+0.2)*(tmhp-thp))*stagedmg1,(10*Rlvl+15+.1*bad)*stagedmg2,(0.05*Rlvl+0.2)*(tmhp-thp)*stagedmg3) --stage1:Maximum (after 1500 units)+Additional Damage. stage2:Minimum Base (Maximum = x2). stage3: Additional Damage
			end
		...
		elseif name == "Karma" then
			...
			elseif spellname == "W" then DmgM = 50*Wlvl+10+.9*ap
			end
		...
		elseif name == "Kassadin" then
			if spellname == "Q" then DmgM = 25*Qlvl+45+.7*ap
		...
		elseif name == "Leblanc" then
			...
			elseif spellname == "R" then DmgM = math.max((100*Rlvl+.6*ap)*stagedmg1,(150*Rlvl+.9*ap)*stagedmg2,(100*Rlvl+.6*ap)*stagedmg3) --stage1:Q Initial or mark. stage2:W. stage3:E Initial or Delayed
			end
		...
		elseif name == "Lissandra" then
			if spellname == "Q" then DmgM = 30*Qlvl+40+.65*ap
		...
		elseif name == "Mordekaiser" then
			...
			elseif spellname == "W" then DmgM = math.max(12*Wlvl+8+.15*ap,(12*Wlvl+8+.15*ap)*6*stagedmg3) --xsec (6 sec). stage3: Max damage
			elseif spellname == "E" then DmgM = 45*Elvl+25+.6*ap
			elseif spellname == "R" then DmgM = (5*Rlvl+19+.04*ap)*tmhp/100 --half Initial and half over 10 sec
			end
		elseif name == "Morgana" then
			if spellname == "Q" then DmgM = 55*Qlvl+25+.6*ap
			elseif spellname == "W" then DmgM = (8*Wlvl+.11*ap)*(1+.5*(1-thp/tmhp)) --x 1/2 sec (5 sec)
		...
		elseif name == "Nautilus" then
			...
			elseif spellname == "W" then DmgM = 10*Wlvl+20+.4*ap TypeDmg = 2 --xhit (bonus)
		...
		elseif name == "Nidalee" then
			...
			elseif spellname == "W" then DmgM = 40*Qlvl+.4*ap -- over 4 sec
		...
		elseif name == "Nocturne" then
			...
			elseif spellname == "E" then DmgM = 40*Elvl+40+ap
		...
		elseif name == "RekSai" then
			if spellname == "Q" then DmgP = 10*Qlvl+5+.2*bad TypeDmg = 2 --(bonus)
			elseif spellname == "QM" then DmgM = 30*Qlvl+30+.7*ap
			elseif spellname == "WM" then DmgP = 40*Wlvl+.4*bad
		...
		elseif name == "Ryze" then
			if spellname == "Q" then DmgM = 30*Qlvl+35+.55*ap+(0.05*Qlvl+0.15)*mmana
			elseif spellname == "W" then DmgM = 30*Wlvl+35+.4*ap+.025*mmana
			elseif spellname == "E" then DmgM = math.max(16*Elvl+34+.3*ap+.02*mmana,(16*Elvl+34+.3*ap+.02*mmana)*4*stagedmg3) --xbounce. stage3: Max damage = initial damage + 6 * 1/2 initial damage = 4 initial damage
			end
		elseif name == "Sejuani" then
			if spellname == "Q" then DmgM = 45*Qlvl+35+.4*ap
			elseif spellname == "W" then DmgM = math.max(((1.5*Wlvl+2.5+.03*ap)*tmhp/100)*stagedmg1,(30*Wlvl+10+.6*ap+(.5*Wlvl+3.5)*mhp/100)/4*(stagedmg2+stagedmg3)) TypeDmg = 1+stagedmg1 --stage1: bonus. stage2-3: xsec (4 sec)
		...
		elseif name == "Shyvana" then
			...
			elseif spellname == "E" then DmgM = math.max((40*Elvl+20+.6*ap)*(stagedmg1+stagedmg3),(2.5*tmhp/100)*stagedmg2) --stage1-3:Active. stage2:Each autoattack that hits debuffed targets
			elseif spellname == "R" then DmgM = 125*Rlvl+50+.7*ap
			end
		elseif name == "Singed" then
			if spellname == "Q" then DmgM = 12*Qlvl+10+.3*ap --xsec
			elseif spellname == "E" then DmgM = 15*Elvl+35+.75*ap+((0.5*Elvl+5.5)*tmhp/100)
			end
		elseif name == "Sion" then
			if spellname == "P" then DmgP = 10*tmhp/100 TypeDmg = 2
			elseif spellname == "Q" then DmgP = 20*Qlvl+.65*ad --Minimum, x3 over 2 sec
			elseif spellname == "W" then DmgM = 25*Wlvl+15+.4*ap+(Wlvl+9)*tmhp/100
			elseif spellname == "E" then DmgM = math.max(35*Wlvl+35+.4*ap,(35*Wlvl+35+.4*ap)*1.3*stagedmg3) --Minimum. stage3: x1.3 if hits a minion
		...
		elseif name == "Skarner" then
			...
			elseif spellname == "Q" then DmgP = (10*Qlvl+10+.4*bad)*(stagedmg1+stagedmg3) QDmgM = (10*Qlvl+10+.2*ap)*(stagedmg2+stagedmg3) --stage1:basic. stage2: charge bonus. stage2: total
			elseif spellname == "E" then DmgM = 35*Elvl+5+.4*ap
		...
		elseif name == "Syndra" then
			if spellname == "Q" then DmgM = math.max(45*Qlvl+5+.6*ap,(45*Qlvl+5+.6*ap)*1.15*(Qlvl-4))
			elseif spellname == "W" then DmgM = 40*Wlvl+40+.7*ap
		...
		elseif name == "Tristana" then
			if spellname == "W" then DmgM = 25*Wlvl+55+.5*ap, (25*Wlvl+55+.5*ap)*2 --max damage, jumping onto max stack explosive charge
			elseif spellname == "E" then DmgP = 10*Elvl+50+(.15*Elvl+.35)*bad+.5*ap,(10*Elvl+50+(.15*Elvl+.35)*bad+.5*ap)*2.2*stagedmg3	--stage3: Max damage/4 auto attack stacks
			elseif spellname == "R" then DmgM = 100*Rlvl+200+ap
			end
		...
		elseif name == "Twitch" then
			if spellname == "P" then DmgT = math.floor((lvl+3)/4 + 1) --xstack xsec (6 stack 6 sec)
		...
		elseif name == "Veigar" then
			...
			elseif spellname == "R" then DmgM = 125*Rlvl+125+ap+.8*tap
			end
		...
		elseif name == "Vi" then
			...
			elseif spellname == "R" then DmgP = 150*Rlvl+1.4*bad --deals 75% damage to enemies in her way
			end
		...
		elseif name == "XinZhao" then
			...
			elseif spellname == "E" then DmgM = 40*Elvl+30+.6*ap
		...
		elseif name == "Zilean" then
			if spellname == "Q" then DmgM = 40*Qlvl+35+.9*ap
			end
