...
print("Recommended to use this script with SAC:R")
...
function OnTick()
	...
	if Rmiss ~= nil and JAnivia.SSettings.Rset.cancelR then
		if JAnivia.SSettings.Rset.keepR then
...
function Harass()
	if JAnivia.HSettings.useQ and Qrdy then
		local qcheck, qtype = Qchecks(Target)
		if qtype ~= nil then
			castQ(qcheck)
		end
	end	
	if JAnivia.HSettings.useE and Erdy then
		local echeck = Echecks(Target)
		if echeck == true then
			castE(Target)
		end
